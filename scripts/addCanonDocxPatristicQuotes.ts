/**
 * Import patristic canon quotes from data/Patristics_on_the_Canon_formatted.docx into CSV
...
function main() {
    const parsed = parsePatristicsCanonDocx();
    ...
    for (const quote of parsed) {
        const subtopics = formatSubtopics(inferCanonSubtopics(quote.quotePlain));
        ...
        if (existing) {
            ...
            if (incomingLonger || quote.quoteMarkdown.includes('**')) {
